...
- **Celulosa + revólver:** un mismo operario ejecuta `celulosa1` y `celulosa2` consecutivamente (5 min cada una). Solo después se programa `espera2`, el revólver de 30 min.
- **Deadline post-revólver:** al terminar el revólver, el tanque queda con deadline para resina; montajes no pueden bloquear a todos los operarios en ese instante.
